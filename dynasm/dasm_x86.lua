...
-- AVX registers (yword sized, but qword and dword accessible).
mkrmap("y", "ymm")
...
--   "s"       stores an 4 bit immediate from the last register operand,
...
local map_op = {
  ...
  -- AVX, AVX2 integer ops (in general, xmm requires AVX, ymm requires AVX2)
...
for name,n in pairs{ paddb = 0xFC, paddw = 0xFD, paddd = 0xFE, paddq = 0xD4,
		     ...
		     punpckldq = 0x62, punpcklqdq = 0x6C, pxor = 0xEF } do
